...
8. **Update documentation.** Update `README.md`, `QUICKSTART.md`, `USER_GUIDE.md`, `CHANGELOG.md`, and the Doxygen comments on anything touched (see Code Documentation below), in a style and level of detail consistent with the existing docs. `README.md`/`QUICKSTART.md`/`USER_GUIDE.md` are user-facing, not developer-facing — update them whenever a step changes what a *user* of the built app can do (a new import format, a new panel or control, a changed default, a control that starts actually doing something it didn't before), not for internal refactors with no user-visible effect. Keep them honest about current scope: describe only what's actually usable in the build today, the same way `USER_GUIDE.md`'s own "What's Not Here Yet" section does — never describe `docs/sound-mind-design.md`'s full aspirational vision as if it already works.
