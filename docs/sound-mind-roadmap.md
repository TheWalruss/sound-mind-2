...
**In progress, scoped in a dedicated planning pass per installment** (confirmed with the user, 2026-09-17), the same pattern the prior three milestones' own installment plans followed: **Installment A - Heal + Soften** (`v0.0.34.1` - see `docs/sound-mind-architecture.md`'s Decision #96) and **Installment B - Smudge + Order/Chaos** (`v0.0.34.2` - Decision #97) are both done. **Clone** remains its own later, final installment for this milestone, not yet started or scoped; the **Tool Configuration Wizard**/**Tool Preset** library (confirmed deferred again, the same as every prior tool-type milestone) stay a separate, later piece of this same overall milestone, not tied to any one tool installment. As shipped:
