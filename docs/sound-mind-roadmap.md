...
**Scoped in a dedicated planning pass per installment** (confirmed with the user, 2026-09-18), the same pattern every prior multi-installment milestone followed: **Installment A - Noise & distortion, core trio plus every growth candidate** (`v0.0.36.1` - see `docs/sound-mind-architecture.md`'s Decision #102) and **Installment B - the rest of Tonal plus the rest of Spectral shaping** (`v0.0.36.2` - Decision #103) are done. **Installment C - Geometric, and Installment D - Space** remain, each needing its own dedicated design pass (Space's own reverb model especially) before starting - not yet started or scoped. As shipped so far:
