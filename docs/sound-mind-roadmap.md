...
**Scoped in a dedicated planning pass per installment** (confirmed with the user, 2026-09-17/2026-09-18), the same pattern the prior milestones' own installment plans followed: **Installment A - a region/mask foundation, plus Lasso** (`v0.0.35.1` - see `docs/sound-mind-architecture.md`'s Decision #99) and **Installment B - Wand + boolean combination** (`v0.0.35.2` - Decision #100) are both done. **Installment C - Rectangle's own rotate handle + Warp** remains, needing its own dedicated design pass (Warp's exact curve-following mechanism in particular) before starting - not yet started or scoped. As shipped so far:
