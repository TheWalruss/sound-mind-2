...
**Scoped in a dedicated planning pass per installment** (confirmed with the user, 2026-09-18), the same pattern every prior multi-installment milestone followed: **Installment A - Noise & distortion, core trio plus every growth candidate** (`v0.0.36.1` - see `docs/sound-mind-architecture.md`'s Decision #102) is done. **Installment B - the rest of Tonal plus the rest of Spectral shaping, Installment C - Geometric, and Installment D - Space** remain, each needing its own dedicated design pass (Space's own reverb model especially) before starting - not yet started or scoped. As shipped so far:
...
**Demo, as shipped so far:** add speckle noise to a Filter layer, then denoise it back out; bitcrush a passage into a lo-fi crunch; add a granular noise texture that stays put across repaints, or switch to Dynamic Speckle and watch it flicker live instead; push a passage into a resonant, decaying smear with Feedback Distortion; wavefold a loud passage into a harsher, more harmonically dense distortion. Geometric, the rest of Tonal/Spectral shaping, and Space remain this milestone's own later installments.
