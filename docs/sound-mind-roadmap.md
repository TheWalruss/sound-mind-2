...
**Scoped in a dedicated planning pass per installment** (confirmed with the user, 2026-09-17), the same pattern the prior milestones' own installment plans followed: **Installment A - a region/mask foundation, plus Lasso** (`v0.0.35.1` - see `docs/sound-mind-architecture.md`'s Decision #99) is done. **Installment B - Wand + boolean combination** and **Installment C - Rectangle's own rotate handle + Warp** remain, each needing its own dedicated design pass before starting (Wand's tolerance/harmonics-aware mechanics and boolean combination's own invocation UI; Warp's exact curve-following mechanism) - not yet started or scoped. As shipped so far:

- **A Lasso selection's own boundary is a plain `sound_mind::core::Path`, reused wholesale rather than a new polygon/mask type** - freehand capture is exactly `PaintController`'s own "gather raw points, refit via curve-fitting on every sample" pattern already established for a paint stroke, so Lasso needed no new capture code at all, and gets translation/JSON serialization for free. `FillOperation`/`PasteOperation` each gained one new optional field narrowing which cells within their own existing bounding box actually get touched - present for a Lasso-shaped Fill/Copy/Cut/Paste, absent (byte-for-byte unchanged behavior) for a plain Rectangle one.
- **A new dockable Selection Configuration Panel** (mirroring the Tool Configuration Panel's own "one dropdown selects behavior" shape) holds the Selection Type dropdown (Rectangle/Lasso for now) - kept separate from the Tool Configuration Panel since Select is its own tool mode, independent of Paint's.
- **A known, deliberate gap**: Mind Shot/Mind Grain capture still always captures/references a selection's own full bounding box, not confined to a Lasso's own shape - out of this installment's scope, not forgotten.

**Demo, so far:** lasso an irregular note shape the Rectangle tool couldn't isolate, then Fill/Copy/Cut/Paste it, confined exactly to that shape. Wand-select a note plus its harmonics in one click, and warp a selected region along a hand-drawn curve, remain this milestone's own later, not-yet-started installments.
