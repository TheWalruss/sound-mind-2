...
**In progress, scoped in a dedicated planning pass per installment** (confirmed with the user, 2026-09-17), the same pattern the prior three milestones' own installment plans followed: **Installment A - Heal + Soften** is done (`v0.0.34.1` - see `docs/sound-mind-architecture.md`'s Decision #96), chosen to go first specifically because both are blur-family effects that could reuse this codebase's own existing Filter-layer blur precedent (`filter_application.cpp`'s "read from a separate buffer, never the one being written" pattern) rather than needing new DSP designed from scratch. **Order/Chaos**, **Smudge**, and **Clone** remain their own later installments, not yet started or scoped; the **Tool Configuration Wizard**/**Tool Preset** library (confirmed deferred again, the same as every prior tool-type milestone) stay a separate, later piece of this same overall milestone, not tied to any one tool installment. Installment A's own shape, as shipped:
...
**Demo, as actually shipped:** paint a stray mark and Heal it away by brushing over it; Soften a busy passage into a smoother wash. The original Demo's own "open the Tool Configuration Wizard and build up a Clone tool step by step" is a later installment of this same milestone, not yet started.
