...
- **The "first fractal field" generator has no earlier implementation to build on** - the design doc's own cross-reference ("the same kind of branching grammar behind fractal MindWave functions") points at `v0.Y.39.1` (Generators, Phase 5), which comes *after* this milestone. This installment plan is what builds that branching-grammar primitive first, for its own narrower (1D-field, not full amplitude/phase) need - the same "simpler first pass, since scheduled before what it'd otherwise lean on" shape this document's own Sequencing principle #4 already names for other milestones.
- **No "direct" filter parameter exists in this codebase yet** to exercise the direct-parameter side of the direct-vs-shape distinction - every filter parameter Filter Layers actually built (blur sigma, median size, directional length/angle, sharpen amount, tone curve points, the frequency gradient) is a *shape* parameter in the design doc's own sense; the design doc's own direct-parameter examples (offset distance/angle, hue-rotation) belong to the still-out-of-scope Geometric/Space filter families. Filter-parameter binding here is therefore only the shape-parameter blend approximation (run the filter once, blend filtered-vs-unfiltered per pixel via the field) - the direct-parameter evaluation path is designed for, but has nothing real to bind to until a later filter-family expansion.
...
### v0.Y.36.1 - Chords/Arpeggiator/Sequencer
...
### v0.Y.37.1 - Loop Mode Live Preview
...
### v0.Y.44.1 - Performance Validation & Hardening
