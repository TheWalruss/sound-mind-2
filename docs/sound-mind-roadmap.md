...
Proposed installment shape (subject to its own scoping/confirmation round when each one starts, same as every other milestone this project has built): **A** - the `MindWave` resource (generator type, params, superposition stack), Project-scoped like `ToolConfiguration`, JSON round-trip, one generator (a sine) proving the field-evaluation pipeline end to end, no UI/binding yet. **B** - the rest of the v1 generator catalogue (periodic's other four shapes, envelope's three, stepped/noise, spatial's four, the fractal field) plus superposition, still Core-only. **C** - layer opacity binding: `Layer` gains an optional MindWave reference, `compositeProject()` evaluates it as a per-cell field during mixing (GPU-aware per the note above - likely its own sub-installments given that scope), and a bare-bones MindWave management panel (generator-type dropdown, plain numeric parameter fields - no Continuous Controls yet, matching Deferred Decision #7's own still-open interaction design) plus a "bind" affordance on the Layers Panel's opacity slider. This alone clears the Demo bar below. **D** - filter-parameter binding (the shape-parameter approximation), likely GPU-aware for the same reason as **C**.
