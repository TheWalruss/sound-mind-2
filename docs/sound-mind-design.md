...
Any numeric parameter of a Filter layer can be bound to a MindWave. Binding a parameter means the parameter genuinely varies, cell by cell: at each canvas position, the MindWave is evaluated at that position, and the result becomes *that cell's own value* of the parameter — the filter's own algorithm is then evaluated using that per-cell value when producing that cell's own output. This is computed exactly, the same way a fixed parameter value is exact — a MindWave-bound parameter is not an approximation or a blend of two whole-image results; it is the real parameter driving a real, independently-computed value at every position. (Binding a MindWave to a *layer's own opacity* — see [Layer opacity](#layer-opacity) above — is a distinct mechanism from filter-parameter binding, not a special case of it, even though both make something vary spatially.) The bound value is the *ceiling* the parameter reaches where the wave is brightest; where the wave is dark, the parameter falls toward its own "no effect" baseline (silence/off, not necessarily zero). This makes a filter spatially selective — sharpening only the loud transients, blurring only the trailing edge of a note, cutting only one frequency band.

Two kinds of parameters differ in *cost*, not in mechanism — both are evaluated exactly, per cell:

- **Pixel-local parameters** — a displacement distance, a displacement angle, a hue-rotation angle, and similar values that describe *where* something is sampled from or *how* it is oriented — need no neighborhood context, so evaluating a different value at every cell costs nothing extra beyond evaluating the MindWave itself.
- **Kernel-shape parameters** — a blur radius, a window/kernel size, and other values that describe the *shape* of a local neighborhood operation — mean every cell effectively gets its own independently-shaped kernel once bound, since the parameter that determines that shape is no longer the same value everywhere. This is real additional computation (no longer expressible as the same fixed/separable algorithm the unbound case uses), not a shortcut — but it is still the exact parameter value at that cell, not a blend or approximation of it.

**Offset distance**, **offset angle**, and **hue-rotation angle** are examples of pixel-local parameters — every other filter parameter should be treated as a kernel-shape parameter unless the same pixel-local reasoning clearly applies. All filter parameters are meant to work this way, so a filter's behaviour when MindWave-bound never needs a special case in the design: every parameter, bound or not, produces the same filter algorithm evaluated with that cell's own parameter value — bound and unbound differ only in whether that value is the same everywhere or genuinely varies.

### Brush parameters

The same binding applies to brush parameters — size, opacity, color/intensity, and the pixel-local parameters described above (offset, angle) all accept a MindWave in place of a fixed value, with the same pixel-local-vs-kernel-shape distinction. This lets a single stroke vary its own strength, width, or hue as it crosses the canvas, driven by the same waveform library used for layers and filters.
...
8. **Mind Grain composite-vs-layer sampling.** A Mind Grain currently samples its source layer's own raw, isolated content — not the full composite (every layer below and including it, blended together with opacity/MindWave binding/filters) as it actually renders at that point in the stack. A future phase should let a Mind Grain sample the composite *as it stood at the layer it's defined on* instead — closer to "what you actually saw when you selected it" than "just that one layer's own pixels underneath everything else." Delegated rather than resolved now (added 2026-09-17, alongside the immediate cross-layer repaint cascade `v0.0.33.3` shipped) — it needs its own scoping pass once a milestone actually depends on it, the same as every other item here.
