...
28. **Playback/Record/Loop default to hidden; Layers gets the same toolbar toggle but defaults to shown - and, unlike before, neither is ever force-reset by switching projects.** Settled in `v0.0.21.1`, confirmed with the user. Previously `setProject()` unconditionally called `->show()` on all three transport panels plus Layers, every single time - the new behavior removes that call entirely for Playback/Record/Loop (they simply stay however the user last left them, defaulting to the hidden state every dock already starts constructed in) and replaces it, for Layers only, with a one-time `layersPanelShownOnce_` flag: shown automatically the *first* time any project exists in a session, then left alone too. This is the same `QDockWidget::toggleViewAction()` mechanism Playback/Record/Loop's own toolbar buttons already used - genuinely no different in kind, just a fourth instance of it, with the opposite default.
   - **A real, headless-testing-only quirk found while testing this**, worth recording since it cost real time to diagnose: `QDockWidget` syncs `toggleViewAction()`'s checked state from its own Show/Hide *events*, which Qt only actually dispatches once a widget's whole ancestor chain is genuinely on screen. `MainWindow` is never `show()`n anywhere in this test suite, so a direct `layersPanel_->show()` call (via `setProject()`) updates `isHidden()` (an explicit per-widget flag, unaffected by ancestor visibility) correctly, but never reaches the toggle action - which can sit permanently desynced from the panel's real visibility for the whole life of a headless test. Confirmed as headless-only (the mechanism itself is Qt's own standard, widely-relied-on one - not something this codebase invented, and not reproducible in a real, actually-shown application): `layersToggleActionShowsAndHidesTheLayersPanel` only verifies the action exists, is checkable, and is in the toolbar - not that toggling it actually changes `isHidden()`, the same category of "can't verify headlessly" limitation `dropEvent()`/`dragEnterEvent()` already have for real OS drag gestures.
