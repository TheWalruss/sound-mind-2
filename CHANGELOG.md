...
- **A new "+ Generate Layer" button** in the Layers panel (alongside + Add Layer/+ Add Filter Layer) - opens a small dialog to pick where the result sits on the Chaos/Order continuum and a seed, then adds a new, ordinary layer filled with an organic, self-similar generated texture. The same seed always reproduces the exact same result.

### Notes

- Only the "Lattice" generator family is implemented so far - the Fractal and Streaming families described in the design document are still future work.

Full regression: `sound-mind-core` 885/885 (6 new), `sound-mind-studio` full suite unaffected (5 new tests, still 0 failures). Builds cleanly on both Arm64 and x64. Doxygen: 0 warnings. See `docs/sound-mind-architecture.md`'s Decision #165.
...
- Three long-standing, pre-existing test failures (`LayerControllerTest`'s opacity-MindWave and blend-mode row-data tests, `MindWaveControllerTest`'s library-refresh test) - none had ever selected the layer row they were inspecting, and the Layers panel's own opacity/MindWave and blend-mode combos have been selection-revealed since the Layers Panel Redesign. No user-visible behavior changed; this only fixes the tests themselves.

Full regression: `sound-mind-core` 871/871 (unaffected), `sound-mind-studio` full suite now genuinely 0 failures (previously 3 pre-existing, now fixed). Builds cleanly on both Arm64 and x64. Doxygen: 0 warnings. See `docs/sound-mind-architecture.md`'s Decision #159.
...
- **Page Up/Page Down** now select the layer above/below the currently selected one in the Layers panel.
- **Shift+Page Up/Page Down** move the selected layer up/down one position in the stack.
- **Ctrl+Page Up/Page Down** nudge the selected layer's opacity up/down by 5% (undoable).

All six are no-ops where they don't apply (nothing selected, already at an end of the stack, or blocked by the locked Background/Equalizer layer).

Full regression: `sound-mind-core` full suite unaffected (840 test cases), `sound-mind-studio` `LayerControllerTest` (3 new), full suite otherwise unchanged. Builds cleanly on both Arm64 and x64. Doxygen: 0 warnings. See `docs/sound-mind-architecture.md`'s Decision #154.
...
- **Pool Layer no longer blocks the app while it processes.** It now runs on a background thread, with its own status-bar cancel button; cancelling leaves the layer's content completely untouched, as if pooling had never been started.
- **New/Open Project and closing the Studio are now disabled while a pool is running**, matching the existing Import Audio/Loop Mode/Recording behavior.

Full regression: `sound-mind-core` 828/828 (3 new), `sound-mind-studio` 201/201 `MainWindowTest` (up from 195). Builds cleanly on both Arm64 and x64. Doxygen: 0 warnings. See `docs/sound-mind-architecture.md`'s Decision #140.
...
- **Export Video no longer blocks the app while it encodes.** It now runs on a background thread, with a small **✕** cancel button in the status bar visible while it's running; cancelling deletes the partial output file rather than leaving a broken one behind.

Full regression: `sound-mind-core` 824/824 (1 new), `sound-mind-studio` 184/184 `MainWindowTest` (up from 180). Builds cleanly on both Arm64 and x64. Doxygen: 0 warnings. See `docs/sound-mind-architecture.md`'s Decision #136.
