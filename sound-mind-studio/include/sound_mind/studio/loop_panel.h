 ...
 *        selection, and "Keep Looping" controls - see
 ...
 * and output device pickers (each with a Refresh button), and "Keep
 * Looping" (moved here from the transport toolbar checkbox `v0.Y.12.1`
 * added as a confirmed stopgap) - narrowed to what this codebase's Loop
 * Mode actually has scope for today: no channel selector, input level
 * meter, or session-saving - none of those are part of this milestone's
 * confirmed scope, unlike the legacy panel.
 *
...
public:
    /// @brief Builds the panel with empty device lists and "Keep Looping"
    ///        unchecked.
    ...
    /// @brief Sets the "Keep Looping" checkbox's displayed state without
...
signals:
    ...
    /// @brief The "Keep Looping" checkbox changed.
